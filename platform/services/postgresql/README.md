...
The base owns the namespace. Each environment overlay owns its chart reference and values. Development generates a disposable `postgresql-auth` Secret; stage and production expect that Secret to be supplied externally in the `postgresql` namespace with keys `postgres-password` and `password`.
